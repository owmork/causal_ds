...
The data was simulated and feel free to check out how it was done. But again, you can skip this part and only follow the explanations.
...
create_corr_df <- function(n = 1e+3, correlation = 0) {
  # Generate random X and Y data
  X <- rnorm(n)
  Y <- correlation * X + sqrt(1 - correlation^2) * rnorm(n)

  # Create a data frame
  df <- data.frame(x = X, y = Y) %>% add_column(sigma = correlation)
...
Plot.dot(filtered, {x: "x", y: "y"}).plot()
