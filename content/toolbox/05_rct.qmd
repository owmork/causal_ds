...
// https://observablehq.com/@mattiasvillani/confidence-interval-for-a-proportion
...
Plot.plot({
  ...theme,
  facet: {data: filtered, x: "name"},
  grid: true,
  color: {legend: true},
  marks: [
    Plot.boxY(filtered, {x: "D", y: "value", fill: "#00C1D4", fillOpacity: .4})
  ]
})
```
...
Until now, we have not assigned units to treatment and control group and actually, we do not want to assign our whole population into treatment and non-treatment. As a matter of fact, in many applications, you are just able to draw a sample from the population and almost never the whole population.
...
$$
outcome = 2*treatment + 0.1*treatment*age + 10*treatment*sex + \epsilon
$$
