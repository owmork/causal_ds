...
It is generally more convenient to use the `lm()` command.
...
Due to confounding, the naive comparison cannot reconstruct our implemented treatment effect (= 0.2, defined when we simulated the outcome variable) and is off. We should get closer if we account for confounding by taking averages treatment effects for men and women separately.
...
2.  **Propensity**: observations are similar if they are equally likely to be treated, i.e. matching variables are used to build a prediction model of how likely an observation is to be in treatment group, regardless whether it actually is in the treatment or control group.
...
#|   - "The closest match is selected to compute treatment effect. Match is indicated by color of data point."
#|   - "Several units are selected and weighted by their distance. Weight is indicated by size of data point."
...
# Create tibble of potential matches with two variables, in/out and weight
df <- tibble(
previous_bill = c(0.5, 2.2, 2, 4.5, 4.2, 1)*100,
age = c(3.4, 3, 4.5, 2.1, 0.8, 1)*10,
m  = as.factor(c(1, 1, 1, 1, 0, 0))
)

# Create focal point
treated <- tibble(
  previous_bill = 2.2*100,
  age = 4.1*10
)

# Initial situation
p1 <- ggplot(df, aes(x = previous_bill, y = age)) +
  geom_point(size = 4) +
    geom_point(aes(x = treated$previous_bill, treated$age),
             pch = 23, size = 7, color = "white", fill= ggthemr::swatch()[5]) +
  coord_cartesian(xlim = c(0, 6*100),
                  ylim = c(0, 6*10)) +
  theme(legend.position = "none") +
  labs(title = "Initial situation")

# In/Out matching
p2 <- ggplot(filter(df, m==1), aes(x = previous_bill, y = age)) +
  geom_point(data = filter(df, m == 0), color = "grey", alpha = .42, size = 4) +
...
euclidean <- function(a, b) sqrt(sum((a - b)^2))
distance <- lapply(1:nrow(df), function(row) {
  vec_df <- c(df$previous_bill[row], df$age[row])
  vec_tr <- as_vector(treated)
...
# Weighted by distance
p3 <- ggplot(df, aes(x = previous_bill, y = age, size = w, color = m)) +
  ggforce::geom_ellipse(aes(x0 = 2.2*100, y0 = 4.1*10, a = 185, b = 18.5, angle = 0),
...
```{r}
#| fig-cap: "Coarsening: each subsample only contains similar units. If there are just a few treated observations, the toal weight of control units is lower for a specific subsample."
...
To avoid assigning extreme weights, a proposed best practice is to trim the sample[^2] and filter out all observations with a propensity score less than 0.15 and higher than 0.85. Doing that,the coefficient is very close to the true treatment effect.

[^2]: https://onlinelibrary.wiley.com/doi/full/10.1002/sim.6607
